...
ZIPR <- function() {
  ...
  pi ~ dunif(0, 1)          # proportion of non-zeros
}
```
...
```r
parameters <- c("alpha", "pi") # which parameters are we interested in getting reported?
...
```r
parameters <- c("alpha", "beta", "pi") # which parameters are we interested in getting reported?
...
DIC is an estimate of expected predictive error (lower deviance is better).
```
So while the model has improved (the DIC is lower by 10 units), the value for $\pi$ hasn't changed much.
...
```r
parameters <- c("alpha", "beta", "pi", "tau") # which parameters are we interested in getting reported?
...
DIC is an estimate of expected predictive error (lower deviance is better).
```
Okay. Again, $\pi$ is still high and seems to be a feature of the data, rather than due to our poor modelling of $\lambda$.
...
It could be that a clutch of chicks is more vocal when it is larger. A single chick may remain silent more often than it would when in a group of siblings (maybe I am extrapolating too much from football supporters on their way to the stadium). Statistically, we can make $\pi$ a function of other predictors, too, in this case of brood size. Let's try.
...
ZIPR4 <- function() {
  for(i in 1:N){ # loop through all data points
    ...
    z[i] ~ dbern(pi[i])
    logit(pi[i]) <- alpha.pi + beta.pi*exp(lgBroodSize[i]) 
  ...
  # remove this: pi ~ dunif(0, 1)          # proportion of non-zeros
  for (j in 1:nnests){
    a[j] ~ dnorm(0, tau)     # random effect for each nest
  }
  tau ~ dgamma(0.001, 0.001) # prior for mixed effect variance
  alpha.pi ~ dnorm(0, 0.01)
  beta.pi  ~ dnorm(0, 0.01)

}
```

```r
parameters <- c("alpha", "beta", "tau", "alpha.pi", "beta.pi") # which parameters are we interested in getting reported?
ZIPR4jags <- jags(OwlsData, inits=inits, parameters, model.file = ZIPR4, n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())
```
...
```r
parameters <- c("alpha", "beta", "tau", "alpha.pi", "beta.pi", "tau.xi") # which parameters are we interested in getting reported?
ZIPR5jags <- jags(OwlsData, inits=inits, parameters, model.file = ZIPR5, n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())
```
...
DIC is an estimate of expected predictive error (lower deviance is better).
```
Oh, that's somewhat of a surprise! The DIC dropped precipitously to just under 2800, i.e. by over 500 units. So I guess the data were substantially overdispersed, not only zero-inflated. Zero-inflation is still prevalent, and we should plot the relationship for $\pi$ to see which values it takes.
...
curve(plogis(-0.75 + 0.46*x), from=min(Owls$BroodSize), to=max(Owls$BroodSize), lwd=1, col="red", las=1, xlab="brood size", ylab="proportion of 0s in the data")
for (i in 1:length(ZIPR5jags$BUGSoutput$sims.list$alpha.pi)){
  thisA <- ZIPR5jags$BUGSoutput$sims.list$alpha.pi[i]
  thisB <- ZIPR5jags$BUGSoutput$sims.list$beta.pi[i]
...
So the proportion of $0$s is between 0.4 and 0.9, and the trend is positive (i.e. more $0$s when there are more siblings). So owl chicks are not like football supporters, it seems.
...
ZIPR5s <- function() {
  ...
  # # # # # # # # # # # # # # # 
  # simulate data here:
  # replace all latent variables L with an S.L (mu, lambda, pi, z), as well as the response:
  for (i in 1:N){ # loop through all data points
    S.SibNeg[i] ~ dpois(S.mu[i]) 
    S.mu[i] <- S.lambda[i]*z[i] + 0.00001 ## hack required for Rjags -- otherwise 'incompatible'-error 
    
    S.z[i] ~ dbern(S.pi[i])
    logit(S.pi[i]) <- alpha.pi + beta.pi*exp(lgBroodSize[i]) 
...
```r
parameters <- c("alpha", "beta", "tau", "alpha.pi", "beta.pi", "tau.xi", "S.SibNeg") # which parameters are we interested in getting reported?
ZIPR5sjags <- jags(OwlsData, inits=inits, parameters, model.file = ZIPR5s, n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())
```
...
The qq-plot looks almost fine, with some overestimation at high values. The standardised residuals (which are actually the quantiles) show should no pattern with expectation, however!
